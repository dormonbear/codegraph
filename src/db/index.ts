...
export class DatabaseConnection {
   ...
   *
   *   - `PRAGMA wal_checkpoint(TRUNCATE)` — (viva-local) fold pending WAL
   *     pages back into the main database file AND reset the -wal file to
   *     zero, so it doesn't grow unboundedly between automatic checkpoints
   *     (auto-fires at 1000 pages by default; large indexAll runs blow past
   *     that). Off-thread, so the extra truncate never blocks the main thread.
   *
  ...
  async runMaintenance(): Promise<void> {
    ...
    // (viva-local) TRUNCATE, not PASSIVE: PASSIVE folds WAL pages back into the
    // db but leaves the -wal file at full size, so a churny long-lived daemon's
    // WAL never shrinks. TRUNCATE also resets the file to zero. It runs on the
    // worker's own connection, so the extra truncate syscall never blocks the
    // main thread (upstream's reason for going off-thread) while still bounding
    // WAL growth (the fork's a11cbd4 goal). See __tests__/db-wal-maintenance.
    await this.runPragmasOffThread(
      ['PRAGMA analysis_limit=1000', 'PRAGMA optimize', 'PRAGMA wal_checkpoint(TRUNCATE)'],
