 ...
 * codegraph daemon gc
 *
 * A `gc` subcommand on the daemon manager above (bare `codegraph daemon` still
 * runs the interactive picker). Reaps leaked/orphaned `codegraph serve --mcp`
 * processes whose supervising host is gone (PID-recycle escapes the watchdog,
 * #277). Daemons are disposable — the next MCP tool call respawns a fresh one —
