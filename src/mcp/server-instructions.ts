...
- **Salesforce — "where is field \`Object__c.Field__c\` read/written?" / "what breaks if I change or remove it?"** → \`codegraph_field_search\` (type + usage summary) / \`codegraph_field_usages\` (per-site, typed read/write/SOQL/LWC-bind) / \`codegraph_field_impact\` (blast radius). \`codegraph_explore\` doesn't cover SObject FIELDS — these do, object-disambiguated so \`Milestone__c.Amount__c\` never collides with \`Task__c.Amount__c\`.
- **Salesforce — "where is OBJECT \`Object__c\` used?" / "what breaks if I rename/delete it?"** → \`codegraph_object_search\` (kind + usage summary) / \`codegraph_object_usages\` (per-site: SOQL FROM, DML, type refs, \`trigger on\`, LWC/VF schema, layouts) / \`codegraph_object_impact\` (blast radius — code usages + Layouts/Validation-Rules + EVERY field it owns with each field's usage count rolled up). Run impact before any object rename/delete: it cascades to all fields, triggers, and layouts.

## Salesforce change-impact chains

- **Field change ("replace field A with B" / retype / delete)**: \`codegraph_field_impact\` \`<Object.Field>\` FIRST — every code site (Apex read/write, SOQL, LWC bind) PLUS the declarative metadata (Layouts, Validation Rules, formula fields) that breaks SILENTLY and grep misses. Then \`codegraph_field_usages\` for the per-site list.
- **Object change ("rename/delete object \`Foo__c\`" / retype)**: \`codegraph_object_impact\` \`<Object>\` FIRST — every code usage (SOQL FROM, DML, \`List<Obj>\`, \`trigger on Obj\`, LWC/VF schema), the declarative metadata (Layouts, Validation Rules, Flows, Permission Sets, Record Types), every field the object owns (each with its usage count), AND the other objects with a lookup/master-detail to it (they orphan on delete) — the full cascade. Then \`codegraph_object_usages\` for the per-site list.
