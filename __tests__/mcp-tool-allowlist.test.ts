...
describe('CODEGRAPH_MCP_TOOLS allowlist', () => {
  ...
  it('exposes the default 7-tool surface when unset', () => {
    ...
    // (viva-local) Following upstream's explore-only default for the generic
    // tools, PLUS the 3 SObject FIELD + 3 SObject OBJECT tools — this fork's
    // reason to exist, so they must be LISTED by default.
    // node/search/callers/callees/impact/files/status stay defined but unlisted.
  ...
  it('treats an empty/whitespace value as unset (default surface)', () => {
    process.env[ENV] = '   ';
    expect(listed()).toHaveLength(7); // (viva-local) explore + 3 field + 3 object tools
